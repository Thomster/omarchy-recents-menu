...
// State-file format for the menu's "recently used" row: a JSON array of
// desktop-entry ids, most-recent-first. Anything else (missing file, stale
// format from a prior version) reads back as no history rather than an error.
function parseRecentAppsFile(raw) {
...
// Apps kept out of the recent list regardless of how often they're launched.
// Matched against both the desktop id and the display label, case-insensitive,
// so it still catches a rename (id changes, label doesn't, or vice versa).
function isExcludedFromRecentApps(appId, label) {
  var text = (String(appId || "") + " " + String(label || "")).toLowerCase()
  return text.indexOf("claude") >= 0
}
...
if (typeof module !== "undefined") {
  module.exports = {
    ...
    parseRecentAppsFile: parseRecentAppsFile,
    isExcludedFromRecentApps: isExcludedFromRecentApps,
    item: item,
