...
Item {
  ...
  // recordRecentApp() in activateIndex(). Persisted so it survives a shell
  // restart; kept a few entries deeper than what's displayed so an uninstalled
  // or excluded app doesn't shrink the visible list below recentAppsDisplayCount.
  readonly property string recentAppsStateDir: Quickshell.env("HOME") + "/.local/state/omarchy/settings"
  readonly property string recentAppsStatePath: root.recentAppsStateDir + "/menu-recent-apps.json"
  property var recentAppIds: []
  readonly property int recentAppsDisplayCount: 5
  readonly property int recentAppsBufferSize: 10
  ...
  function rowListHeight(_serial, _count, _filter, _divider) {
    ...
    for (var i = 0; i < displayModel.count; i++) {
      ...
      // "root" reuses the same divider as "drilldown" -- it marks the row
      // just below the recent-apps list on the root menu (see rebuildDisplay).
      if ((row.section === "drilldown" || row.section === "root") && row.section !== previousSection) total += root.dividerHeight
  ...
  function isExcludedFromRecentApps(appId, label) {
    return MenuModel.isExcludedFromRecentApps(appId, label)
  }

  // Up to recentAppsDisplayCount rows for the ids in recentAppIds that still
  // resolve to an installed, non-excluded app -- built the same way as a
  // normal Apps row (root.displayRow off the real "apps.<id>" item) so icon,
  // launch and uninstall all behave identically to the Apps submenu.
  function recentAppRows() {
    var rows = []
    var ids = root.recentAppIds
    for (var i = 0; i < ids.length && rows.length < root.recentAppsDisplayCount; i++) {
      var entry = root.item("apps." + ids[i])
      if (!entry || entry.kind !== "app") continue
      if (root.isExcludedFromRecentApps(entry.appId, entry.label)) continue
      rows.push(root.displayRow(entry, entry.description, entry.order, "recent"))
    }
    return rows
  }

  function saveRecentApps(ids) {
    var path = root.recentAppsStatePath
    recentAppsSaveProc.command = ["bash", "-c",
      "mkdir -p " + Util.shellQuote(root.recentAppsStateDir)
      + " && printf '%s' " + Util.shellQuote(JSON.stringify(ids)) + " > " + Util.shellQuote(path)]
    recentAppsSaveProc.running = true
  }

  // Called from activateIndex() for every app launched off the Apps submenu
  // (recent rows included, so re-launching one just moves it back to the
  // front). Excluded apps are never recorded, not merely filtered at display
  // time, so they can't occupy one of the buffer's limited slots.
  function recordRecentApp(appId, label) {
    var id = String(appId || "")
    if (!id || root.isExcludedFromRecentApps(id, label)) return
    var next = [id].concat(root.recentAppIds.filter(function(existing) { return existing !== id }))
    next = next.slice(0, root.recentAppsBufferSize)
    root.recentAppIds = next
    root.saveRecentApps(next)
  }
  ...
  function rebuildDisplay() {
    ...
    if (query) {
      ...
      // Recent apps only ever show on the unfiltered root menu -- not inside
      // a search, and not on Apps itself, where the same apps already list
      // alphabetically just below.
      var recentRows = active === "root" ? root.recentAppRows() : []

      for (var j = 0; j < root.itemOrder.length; j++) {
        var child = root.item(root.itemOrder[j])
        if (!child || child.parent !== active) continue
        if (!root.isVisible(child)) continue
        // A distinct, non-empty section on every row below the recents marks
        // where the divider goes (see rowListHeight and the section delegate
        // below); left at the default "" when there's no recent row above.
        rows.push(root.displayRow(child, child.description, child.order, recentRows.length > 0 ? "root" : ""))
      }
      ...
      if (recentRows.length > 0) rows = recentRows.concat(rows)
    }
  ...
  function activateIndex(index, fromPointer) {
    ...
    if (row.kind === "menu" || row.kind === "link") {
      ...
      var appId = row.appId
      var label = row.label
      root.recordRecentApp(appId, label)
  ...
  // Fire-and-forget write for recordRecentApp(); root.recentAppIds is already
  // updated in memory by the time this runs, so nothing here waits on it.
  Process {
    id: recentAppsSaveProc
  }
  ...
  // Recent-apps history (see recordRecentApp/recentAppRows). Missing file
  // just means no history yet, not an error.
  FileView {
    id: recentAppsFile
    path: root.recentAppsStatePath
    watchChanges: true
    printErrors: false
    onLoaded: root.recentAppIds = MenuModel.parseRecentAppsFile(text())
    onLoadFailed: root.recentAppIds = []
    onFileChanged: reload()
  }
        ...
        Item {
          ...
          ListView {
            ...
            section.delegate: Item {
              ...
              height: (section === "drilldown" || section === "root") ? root.dividerHeight : 0
              visible: section === "drilldown" || section === "root"
